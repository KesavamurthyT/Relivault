...
contract EfficientDisasterRelief is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    struct Claim {
        ...
        bool cidValidated;   // CID validation status
        address ngoVerifier; // NGO that verified high-risk claim (address(0) if none)
        uint32 validationTimestamp; // When validation occurred (packed for gas efficiency)
    }
    ...
    /**
     * @notice Submit a relief claim with IPFS CID validation.
    ...
    function getClaim(uint256 id) external view returns (
        address, uint256, uint256, ClaimStatus, string memory, bool, address, uint32
    ) {
